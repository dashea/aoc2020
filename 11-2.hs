 ...
 where
    ...
    doSquare y x s@(Seat _) =
        let directions = [(subtract 1, id), (subtract 1, subtract 1), (id, subtract 1), ((+1), subtract 1),
            ...
            adjacents  = mapMaybe (visibleSeat (x, y)) directions
    ...
    visibleSeat :: Point -> (Int -> Int, Int -> Int) -> Maybe Point
    visibleSeat p dp =
        let p' = uncurry bimap dp p
         in case getSquare r p' of
              Nothing -> Nothing
              Just Floor -> visibleSeat p' dp
              Just (Seat _) -> Just p'
