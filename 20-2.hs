...
findMonsters :: Grid Pixel -> Maybe (Array (Int, Int) Pixel)
findMonsters grid =
    let borderless = map (map removeBorders) grid
        sea = combineGrid borderless
        allSeas = map snd $ allOrientations (0, sea)
        seasWithMonsters = map (markMonsters . toArray) allSeas
     in head $ filter anyMonster seasWithMonsters
 where
    anyMonster :: Array (Int, Int) Pixel -> Bool
    anyMonster input = elem Monster $ Array.elems input
...
main = do
    ...
    withMonsters <- maybe (fail "no monsters found") return $ findMonsters grid
