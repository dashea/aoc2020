...
getNeighbors s p =
 ...
 where
    applyDirection (x, y, z, w) (dx, dy, dz, dw) = (dx x,dy y,dz z,dw w)

addBorders :: Space -> Space
addBorders s =
    let ((xmin, ymin, zmin, wmin), (xmax, ymax, zmax, wmax)) = Array.bounds s
        newRange = ((xmin - 1, ymin - 1, zmin - 1, wmin - 1), (xmax + 1, ymax + 1, zmax + 1, wmax + 1))
...
parsePlane input = do
    ...
    return $ Array.listArray ((0, 0, 0, 0), (rowLen - 1, length plane - 1, 0, 0)) $ concat plane
 where
