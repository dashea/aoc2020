...
instance Ord Point where
    compare (Point x1 y1 z1 w1) (Point x2 y2 z2 w2)
      | w1 /= w2 = w1 `compare` w2
      | z1 /= z2 = z1 `compare` z2
      | y1 /= y2 = y1 `compare` y2
...
instance Ix Point where
    range (Point x1 y1 z1 w1, Point x2 y2 z2 w2) =
        [Point x y z w | w <- [w1..w2], z <- [z1..z2], y <- [y1..y2], x <- [x1..x2]]

    index (Point xmin ymin zmin wmin, Point xmax ymax zmax wmax) (Point x y z w) =
        let xidx = index (xmin, xmax) x
            yidx = index (ymin, ymax) y
            zidx = index (zmin, zmax) z
            widx = index (wmin, wmax) w
            xSize = rangeSize (xmin, xmax)
            ySize = rangeSize (ymin, ymax)
            zSize = rangeSize (zmin, zmax)
         in xidx + (yidx * xSize) + (zidx * xSize * ySize) + (widx * xSize * ySize * zSize)

    inRange (Point xmin ymin zmin wmin, Point xmax ymax zmax wmax) (Point x y z w) =
        (x >= xmin) && (x <= xmax)
        && (y >= ymin) && (y <= ymax)
        && (z >= zmin) && (z <= zmax)
        && (w >= wmin) && (w <= wmax)
...
getNeighbors s p =
 ...
 where
    applyDirection (Point x y z w) (dx, dy, dz, dw) = Point (dx x) (dy y) (dz z) (dw w)

addBorders :: Space -> Space
addBorders s =
    let (Point xmin ymin zmin wmin, Point xmax ymax zmax wmax) = Array.bounds s
        newRange = (Point (xmin - 1) (ymin - 1) (zmin - 1) (wmin - 1), Point (xmax + 1) (ymax + 1) (zmax + 1) (wmax + 1))
...
parsePlane input = do
    ...
    return $ Array.listArray (Point 0 0 0 0, Point (rowLen - 1) (length plane - 1) 0 0) $ concat plane
 where
